...
try:
    ...
    st.error("Add GROQ_API_KEY to .streamlit/secrets.toml - get a free key at console.groq.com")
    st.stop()
...
def render_quiz(quiz: dict) -> None:
    ...
    if not st.session_state.quiz_done:
        ...
        if not st.session_state.quiz_correct:
            st.error(f"Wrong. Correct: {opts[correct] if opts else ''}")
...
# ── Search row — wrapped in form so Enter key triggers Explore
...
with right:
    ...
    with tab_code:
        ...
        st.markdown(
            comp.console_out(
                st.session_state.output or "// Ready. Run code to see output.",
    ...
    with tab_chat:
        ...
        # Chat input wrapped in form so Enter key sends message
        ...
        if data and not st.session_state.chat:
            ...
            st.markdown(
                '<div style="margin-top:0.5rem;font-size:0.68rem;color:#6b7094;'
